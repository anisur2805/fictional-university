...
class Like {
  ...
  events() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on("click", ".like-box", this.dispatchLike.bind(this));
  }
  dispatchLike(e) {
    e.preventDefault();
...
class Notes {
  constructor() {
    this.events();
    this.createNoteBtn = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#create-note');
  }
  events() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).on("click", ".delete-note", this.deleteNote.bind(this));
  ...
  makeNoteEditable(nodeId) {
    nodeId.find(".note-title-field, .note-body-field").removeAttr("readonly").addClass("note-active-field");
    nodeId.find(".update-note").addClass("update-note--visible");
    nodeId.find(".edit-note").html('<i class="fa fa-times" aria-hidden="true"></i> Cancel');
    nodeId.data("state", "editable");
  }
  makeNoteReadonly(nodeId) {
    nodeId.find(".note-title-field, .note-body-field").attr("readonly", "readonly").removeClass("note-active-field");
    nodeId.find(".update-note").removeClass("update-note--visible");
    nodeId.find(".edit-note").html('<i class="fa fa-pencil" aria-hidden="true"></i> Edit');
    nodeId.data("state", "readonly");
  }
  editNote(e) {
    var nodeId = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).parents("li");
  ...
  deleteNote(e) {
    var nodeId = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).parents("li");
    var nodeId = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).closest("li");
  ...
  toggleCreateNoteBtn() {
    var $title = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.new-note-title').val().trim();
    var $content = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.new-note-body').val().trim();
...
class Search {
  constructor() {
    ...
    this.resultDiv = document.querySelector(".search-overlay__results .container");
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(globalObj.rest_url + "university/v1/search?term=" + this.searchField.value);
      const results = response.data;
      this.resultDiv.innerHTML = `
                <div class="row">
                    <div class="one-third">
                        <h2 class="search-overlay__section-title">General Information</h2>
                        ${results.generalInfo.length ? '<ul class="link-list min-list">' : `<p>No results for ${this.searchField.value}</p>`}
                    ...
                    <div class="one-third">
                        ...
                        ${results.professors.length ? '<ul class="professor-cards link-list min-list">' : `<p>No results for ${this.searchField.value} <a href="${globalObj.root_url}/professors">View All Professors</a></p>`}
                        ...
                        ${results.events.length ? '<ul class="event-summary min-list">' : `<p>No results for ${this.searchField.value} <a href="${globalObj.root_url}/events">View All Events</a></p>`}
                    ...
                    <div class="one-third">
                        ...
                        ${results.campuses.length ? '<ul class="link-list min-list">' : `<p>No results for ${this.searchField.value} <a href="${globalObj.root_url}/campus">View All Campuses</a></p>`}
                        ${results.campuses.map(item => `<li><a href="${item.link}">${item.title}</a></li>`).join("")}
                        ${results.campuses.length ? "</ul>" : ""}
                    
                        <h2 class="search-overlay__section-title">Programs</h2>
                        ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No results for ${this.searchField.value} <a href="${globalObj.root_url}/campus">View All Campuses</a></p>`}
